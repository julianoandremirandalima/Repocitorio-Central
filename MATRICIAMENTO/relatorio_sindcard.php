...
$id = $_REQUEST["id"];
$btstatus = $_REQUEST["btstatus"];
if($btstatus=="sim")
{
	$alterastatus = $con->prepare("UPDATE vendas SET statusparcela = '$status' WHERE id_venda = '$id'");
	$alterastatus->execute();
}
?>
...
<!-- End Header -->
  ...
  <a  href="relatorio_sindcard_excel.php?mes=<?=$mes?>&ano=<?=$ano?>&filtrar=sim" target="_blanck"> <button type="button" class="btn btn-warning btn-sm" >
        ...
        <tbody>
            ...
            $valorvenda = $row["valortotal"];
            $chavevendabd = $row["chave_venda"];
            $datarealvenda = $row["datarealvenda"];
            $iddaloja = $row["id_loja"];

            $selectdatareal = $con->prepare("SELECT * FROM vendas WHERE chave_venda = '$chavevendabd'");
            $selectdatareal->execute();
            $arraydatarealchave = $selectdatareal->fetch();
            $datarealchave = $arraydatarealchave["datarealvenda"]; 

            $cor = "table-success";
            /*
            //ver duplicados===================
            $selectduplicados = $con->prepare("SELECT * FROM vendas WHERE id_loja = '$iddaloja' AND valortotal = '$valorvenda' AND datarealvenda = '$datarealvenda'");
            $selectduplicados->execute();
            while($pegar = $selectduplicados->fetch())
            {
              $chavedup = $pegar["chave_venda"];
              if($chavevendabd!=$chavedup)
              {
                $cor = "table-success";
              }
              else
            ...
            */
				...
				?>
				<tr id="<?php echo $row["id"]; ?>" class="<?=$cor?>">
